class ScatterPlot {
    ...
    init() {
        ...
        self.yaxis_group = self.chart.append('g').attr('transform', `translate(" + 0 + "," + (self.inner_height - self.config.margin.top) + ")`);
    ...
    render() {
        ...
        circles
            .on('mouseover', (e,d) => {
                d3.select('#tooltip')
                    .style('opacity', 1)
                    .html(`<div class="tooltip-label">${d.station}</div>(${d.reviewAverage}, ${d.hotelMinCharge}, ${d.reviewCount})`);
            })
            .on('mousemove', (e) => {
                const padding = 10;
                d3.select('#tooltip')
